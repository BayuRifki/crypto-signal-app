...
export const getKlines = async (symbol: string, interval: Interval, limit = 500): Promise<Candle[]> => {
  const url = `${BASE}/api/v3/klines?symbol=${symbol}&interval=${interval}&limit=${limit}`;
  const data = await fetchJson<(string | number)[][]>(url);
  return data.map(mapKline);
};

export const getTicker = async (symbol: string): Promise<Ticker24h> => {
...
export const getUsdtSymbols = async (): Promise<SymbolInfo[]> => {
