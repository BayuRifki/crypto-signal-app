...
description: Use ONLY for end-to-end fullstack feature work on TypeScript-first stacks — Next.js App Router, React Server Components, tRPC/Hono APIs, PostgreSQL with Drizzle, Turborepo/pnpm monorepos, Zod-validated contracts shared between backend and frontend, edge runtimes, real-time/WebSocket, and AI-native integration (Vercel AI SDK, RAG, streaming useChat). Triggers on requests like "build a feature end-to-end", "design the data model and API together", "implement server components with tRPC", "set up auth across DB/API/UI", "add an LLM/RAG pipeline", or "ship a complete stack change". Skips single-layer asks (pure UI tweaks, DB-only migrations) unless the user explicitly wants the full pipeline.
---
