...
const c = generateDemoCandles('volatile', 400, 'VOL1');
...
for (const t of bt.trades) {
  ...
  console.log(`i=${t.index} act=${t.action} ent=${t.entry.toFixed(2)} SL=${t.stopLoss.toFixed(2)} TP=${t.takeProfit.toFixed(2)} pnl=${t.pnlPct.toFixed(2)}% exit=${t.exitReason} conf=${t.confidence} reg=${sig?.regime} bias=${sig?.regimeBias} rsi=${sig?.rsiValue?.toFixed(1)} bb=${sig?.bbPos?.toFixed(0)} rvol=${sig?.rvol?.toFixed(2)} adx=${sig?.adx?.toFixed(1)} atrPct=${sig ? (sig.risk.atr / sig.price * 100).toFixed(2) : 'n/a'}% score=${t.score}`);
}
